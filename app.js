...
function newCase(data={}){
  return {
    id: uid('c'), bed: data.bed || '', initials: (data.initials || '').toUpperCase(),
    ...
    address: data.address || '', geoError: data.geoError || ''
  };
}
function newNurse(data={}){
  return {
    id: uid('n'), name: data.name || '', shift: data.shift || 'DAY',
...
function renderSchedule(){
  ...
  for(const p of state.schedule){
    assigned+=p.visits.length;totalTravel+=p.totalTravel;
    ...
    head.innerHTML=`<h3>${p.nurse.name||'Unnamed nurse'}</h3><span>${p.visits.length} visit${p.visits.length===1?'':'s'}</span>`;
    ...
    for(const e of p.events){
      ...
      let detail=e.label;
      if(e.type==='visit'){
        const purposes=[...(e.case.purposes||[]),...(e.case.other?[e.case.other]:[])].join(', ')||'Visit';
        const region=regionForPostal(e.case.postal);
        detail=`${e.label}<small>${purposes} · ${e.case.postal} · ${e.case.duration} min · ${e.case.type} · ${region}</small>`;
      }
    ...
    const visits=p.events.filter(e=>e.type==='visit');
    ...
    const fifthAllowed=p.visits.length===5&&p.visits.some(v=>v.case.category==='Short/simple'||v.case.duration<=30);
    if(p.visits.length===5&&!fifthAllowed)warnings.push(`${p.nurse.name||'Unnamed nurse'}: fifth visit has no short/simple 15–30 minute visit.`);
    if(p.visits.length>5)warnings.push(`${p.nurse.name||'Unnamed nurse'}: more than five visits assigned.`);
...
function exportCSV(){
  ...
  const rows=[['Nurse','Shift','Start','End','Activity','Patient','Postal Code','Purpose','Duration','Visit Type']];
  for(const p of state.schedule){
    for(const e of p.events){
      if(e.type==='visit'){
        const purpose=[...(e.case.purposes||[]),...(e.case.other?[e.case.other]:[])].join('; ');
        rows.push([p.nurse.name,p.nurse.shift,fmtTime(e.start),fmtTime(e.end),'Visit',patientLabel(e.case),e.case.postal,purpose,e.case.duration,e.case.type]);
...
function save(show=true){
  const data={cases:state.cases,nurses:state.nurses,date:$('#assignmentDate').value,base:$('#basePostal').value,token:$('#oneMapToken').value};
...
function load(){
  try{
    ...
    if(d){
      state.cases=(d.cases||[]).map(x=>newCase({...x,id:undefined}));
      state.nurses=(d.nurses||[]).map(x=>newNurse({...x,id:undefined}));
  ...
  updateDayRule();renderCases();renderNurses();
}
